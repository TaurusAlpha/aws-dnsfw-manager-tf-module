...
data "external" "get_managed_guardduty_rslvr_id" {
  program = ["sh", "-c", "aws route53resolver list-firewall-domain-lists | jq '.FirewallDomainLists[] | select(.Name == \"AWSManagedDomainsAmazonGuardDutyThreatList\") | {id: .Id}'"]
}